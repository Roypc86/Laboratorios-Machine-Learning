...
"""
@author: Blopa
@author: Roy Esteban Padilla Calderón B85854
"""

'''
Respuestas en el documento adjunto Respuestas.pdf
'''
import enum
import math
import numpy as np
...
### Listas de algunos parámetros utilizados
LIST_DEFAULT_ALPHA_LR = [0, 0.001, 1e-4, 1e-6, 8e-7, 1e-5]
LIST_DEFAULT_GAMMA_DISCOUNT = [0, 0.001, 0.5, 1e-4]
LIST_DEFAULT_EPS_GREEDY = [0, 0.001, 0.5, 0.65, 0.7]
LIST_DEFAULT_ETA_DECAY = [0, 0.001, 1e-4, 0.001, 0.009, 0.002,  0.006, 0.000004, 0.001]


### Default values
DEFAULT_ALPHA_LR = LIST_DEFAULT_ALPHA_LR[-1]
DEFAULT_GAMMA_DISCOUNT = LIST_DEFAULT_GAMMA_DISCOUNT[-1]
DEFAULT_EPS_GREEDY = LIST_DEFAULT_EPS_GREEDY[-1]
DEFAULT_ETA_DECAY = LIST_DEFAULT_ETA_DECAY[-1]
...
class Agent():
    ...
    def __init__(self, seed : int, state_dims : list, actions : list, learning_rate : float, discount_factor : float, eps_greedy : float, decay : float):
        # Use self.prng any time you require to call a random funcion
        self.prng = random.Random(seed)
        self.state_dims = state_dims # [0] => altura / [1] => ancho/ [2] => llave/ [3] [4] [5] = bonus 
        self.actions = actions
        self.learning_rate = learning_rate
        self.discount_factor = discount_factor
        self.eps_greedy = eps_greedy
        self.decay = decay # aplica únicamente al eps_greedy
        # inserción de la dimensión de las acciones
        shape = list(state_dims)
        shape.insert(2, 4)
    ...
    def simulation(self, env : "Maze"):
        ...
        step_black_finish = False
        steps = 0
        
        while(not env.is_terminal_state() and not step_black_finish): # Mientras no se esté en un estado terminal
            step_black_finish = self.step(env, True)
            steps += 1
    ...
    def step(self, env : "Maze", learn=True):
        ...
        init_position = env.get_state()
        reward = 0
        if learn and random < self.eps_greedy:
            # elegir una acción al azar
            move = self.prng.choice(self.actions)
            reward, t_s_a = env.perform_action(move)
                        
            if reward > 0:
                max_future_a = np.unravel_index(np.argmax(self.qtable[t_s_a[:2]]), self.qtable[t_s_a[:2]].shape)
                # inserción de la acción a ejecutar en los movimientos
                init_position = list(init_position) 
                init_position.insert(2, move)
                t_s_a = list(t_s_a)
                t_s_a.insert(2, max_future_a[0])
                # Actualización del valor en el q-table
                self.qtable[ tuple(init_position)] = self.qtable[tuple(init_position)] + self.learning_rate * (reward + self.eps_greedy * ( self.qtable[tuple(t_s_a)] - self.qtable[tuple(init_position)])) # Q(s,a) + α(R(T(s,a)) + γ maxa’Q(T(s,a),a’) − Q(s,a)) 
        else: # seleccionar la mejor acción conocida
            action = np.unravel_index(np.argmax(self.qtable[init_position[:2]]), self.qtable[init_position[:2]].shape)[0]
            reward, t_s_a = env.perform_action(action)
